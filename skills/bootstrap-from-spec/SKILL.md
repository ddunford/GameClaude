...
3. **Decompose the current milestone to task detail — via `plan-milestone`.** Owners from `ROSTER.md`, acceptance criteria, a named test-plan (or `[no-test: <reason>]`), and a **tool/skill** hint (the owning skill + the specific tool/route the owner drives, per `plan-milestone`'s task shape) on every task; build≠verify (doctrine 1), so any visual / networked / client-reachable task names a *separate* verify and judge owner. Order by dependency and mark editor-mutating tasks — they serialize; only read-only owners parallelize (`guides/tooling-ue.md`). Once decomposed, `asset-breakdown` produces the phase's asset requirements (the HAVE / ACQUIRE / GENERATE bill-of-materials that feeds `fab-acquire` → `ingest-asset` → build).
...
5. **Hand off.** Commit the roadmap + the current phase file (the task **detail**), and seed `TODO.md` (repo root) as **the live driver** — the ordered task queue of the open phase with a status marker on each item, a link to its detail in the phase file, and the single "you are here" line at the top. `team-execute` reads `TODO.md`, picks the next open item, and keeps it current; `plan-milestone` re-runs on the next milestone as it opens; the phase-close re-scope review (SOP close stage) rewrites the forecast at each gate. Then stop — this skill runs once.
...
- `plan/<current-milestone>.md` — the current milestone decomposed to tasks (via `plan-milestone`) — the task detail.
- `TODO.md` (repo root) — the live task queue + status + "you are here", driven by `team-execute`.
- `plan/STATE.md` — the cold-session orientation entry point, naming `TODO.md` as the live driver and pointing at it for what's next.
