...
1. **The open `plan/<milestone>.md`** — the current phase, the task list, and the **Current state** ("you are here") line at its top. This is the primary anchor.
2. **`.claude/CLAUDE.md`** (the constitution) — the doctrine and the phase gates. Do not re-litigate what it settles.
3. **`ROSTER.md`** — who owns what, and which agents are authored vs stubbed.
4. **`guides/production-pipeline.md`** — the SOP loop, the phase this milestone sits in, and the activation schedule (§3.5) for which disciplines are live now.
5. **`TODO.md`** (repo root), if no plan phase is open — between milestones it is the driver.
...
3. **Reconcile the "you are here" line against reality.** If the plan's current-state line disagrees with the task markers or the git state, the line is stale — correct it first (a stale line is the worst bug).
