...
13. **Guides, skills, and toolsets are living — improve them the moment you learn, and never lose the improvement.** The moment you learn something the docs should have told you — an engine quirk, a gotcha, a mistake a discipline should never repeat — write it into the owning guide **in the same session**, as present-tense fact (an engine-behaviour lesson → `guides/unreal-engine.md` with a source citation, or `[verify]` until confirmed; a process lesson → the relevant guide/agent). The same rule governs the studio's *own* tools: when, mid-build, a skill is weak or missing or a tool is missing or fiddly, improve or build it **in the same session** rather than working around it silently — gap-driven and just-in-time, closing every gap you hit (the toolkit philosophy in `guides/tooling-ue.md`). **Any such improvement to a guide, skill, or toolset is committed and pushed promptly, the same session** — uncommitted skill/tool work is a liability a session-limit or crash destroys. A lesson or a tool that lives only in a session, a commit message, or someone's memory is lost, and the studio repeats the mistake. This is how the knowledge base and the tooling compound instead of rotting.
