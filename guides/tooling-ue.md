...
| **Screenshots / views** (top-down, elevation, eye-level, silhouette) | our toolkit — a capture tool wrapping SceneCapture2D/HighResShot. **Requirement:** handle the cold-start (first render can be black — prime it), return the image path, expose the canonical-view battery in one call | ⧗ harden |
